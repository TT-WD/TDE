...
def get_transform_tooth(origin_size, output_size, rot=0):
    """
    General image processing functions
    """
    # Generate transformation matrix
    #牙齿数据集变换有两种情况，一种可能是方形到方形，一种可能是原始图高宽不等，按最长边比例缩放
    t = np.zeros((3, 3))
    scale=float(max(output_size[0],output_size[1]))/max(origin_size[0],origin_size[1])
    t[0, 0] = scale
    t[1, 1] = scale
    # t[0, 2] = output_size[1] * (-float(center[0]) / h + .5)
    # t[1, 2] = output_size[0] * (-float(center[1]) / h + .5)
...
def transform_pixel_tooth(pt,input_size,output_size,invert=0,rot=0):
    #该方法只用于将关键点 pt从input_size的位置调整到output_size的尺寸,input_size为[w,h]
    t = get_transform_tooth(input_size, output_size, rot=rot)
    if invert:
        t = np.linalg.inv(t)
    new_pt = np.array([pt[0] - 1, pt[1] - 1, 1.]).T
    new_pt = np.dot(t, new_pt)
...
def transform_preds_tooth(coords, origin_size, target_size):
    # 遍历所有关键点，把其映射到原始图像的尺寸
    for p in range(coords.size(0)):
        coords[p, 0:2] = torch.tensor(transform_pixel_tooth(coords[p, 0:2], origin_size, target_size, 1, 0))
    return coords
...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        #sf是一个放缩比例，new_size 是最大边边长，为后续扩展成方形做准备,整体是先放缩再截取
        ...
        if new_size < 2:
            ...
            #img=Image.fromarray(img).resize((new_wd, new_ht), Image.BILINEAR)
            img=np.array(Image.fromarray(np.uint8(np.clip(img, 0, 255))).resize((new_wd, new_ht), Image.BILINEAR))
    ...
    new_img=Image.fromarray(np.uint8(np.clip(new_img , 0, 255))).resize(output_size, Image.BILINEAR)
